...
#        self.wordlisttf = pickle.load(open('./data/wordlist.pkl'))  # 出现频率最高的2000个单词
        ...
        self.INCNAME = re.compile(u"\S+(有限公司|酒店|银行|集团|厂|研究中心|研究所|学校|旅行社|中心/s|分?公司|研发中心|技术部|事.部|招聘|商务平台)") 
        ...
        self.DEMAND = re.compile(u"熟悉|熟练|具有|善于|懂得|掌握|具备|能够|优先|不少于|不超过|至少|团队.作|良好的|工作经验|开发经验|实习经历|能力强|富有|学历|经验|喜欢|较强的.{2,8}能力|相关专业|相关学历|者优先|精通|了解|及以上|技术全面|.强的责任心|[能有]独立|英文流利")
        self.DUTY = re.compile(u"跟进|协助|负责|配合|其他工作|领导交办的|对.+提供|审核|参与|为.+提出|日常.+工作|指导|对.+进行|为.+提供|跟进|拓展|运营|用户|客户|协调|拟写|通过|协同|完成|沟通|需求|秘书.{2,5}翻译")
    ...
    def regular_exp(self):
        ...
        if not res:
            exp = re.search(u"(\d-)?\d{1,2}年(工作|开发|项目)?经[历验]|(不少于)?([半\d]年)及?(以上)?经[历验]|经[历验]\s?(\d-)?\d{1,2}年",' '.join(self.regular_jobtag()))
            if exp:
                res.add(exp.group())
            else:
    ...
    def regular_duty(self):
        ...
        if len(pos)>0:
            ...
            for i in xrange(len(linelist)):
                line = linelist[i]
                if self.START_DUTY.search(line) or self.lineindex[line]==1 or (re.search(u".年来|谢谢|请在|公司介绍|举报|收藏|岗位职责",line) and not re.search(u"了解",line)):continue
    ...
    def regular_demand(self):
        ...
        if len(pos)>0:
            tmppos = pos[-1].span()[0]
            if re.search(u"具有|具备",jdStr[tmppos-5:tmppos+5]) or re.search(u"证书|证",jdStr[tmppos:tmppos+8]):pos.pop()
            if pos:
                linelist =[ re.sub("[\s ]+"," ",line) for line in jdStr[pos[-1].span()[1]:].split("\n") if len(line)>2]
            else:
                linelist = []
    ...
    def regular_pubtime(self):
        ...
        for line in self.linelist:
            ...
            if findpos and len(line)>6:
                findtime = re.search(u"\d{2,4}.\d{1,2}.\d{1,2}日?",line)
                if findtime:
            ...
            if findpos and len(line)>6:
                findtime = list(re.finditer(u"\d{2,4}.\d{1,2}.\d{1,2}日?",line))
                if findtime:
            ...
            for line in self.linelist:
                ...
                for item in jieba.cut(line):
                    if item in self.firmnames:
                        res.add(item)
                if len(res)>0:break
        res = filter(lambda x:len(x)>1 and not re.search(u'^\d|参与|负责|协助|的|[，。:：！？，薪]|招聘|诚聘',x),list(res))
